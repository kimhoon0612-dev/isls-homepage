...
export default async function Page() {
  ...
  return (
    <SubLayout title="By-laws" menuTitle="Membership" menuItems={menuItems} currentPath="/membership/bylaws">
      {!isAuthenticated ? (
        <MembersOnlyLogin message="Please sign in to access the ISLS By-laws." />
      ) : (
        <div className="bg-white border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-10 animate-fade-in">
          <h3 className="text-3xl font-serif font-bold text-gray-900 border-b-2 border-primary pb-4 mb-8">ISLS By-laws</h3>
          <div className="prose max-w-none prose-lg text-gray-700 font-sans leading-relaxed">
            <p className="bg-gray-50 p-6 rounded-2xl border border-gray-100">The official ISLS By-laws document is currently being updated for the new website. If you require immediate access to the current By-laws, please contact the ISLS secretariat office.</p>
          </div>
        </div>
      )}
    </SubLayout>
